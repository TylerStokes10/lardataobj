...
public:
  ...
  UniformNoiseCreator(std::string name, float RMS, float pedestal = 0.)
    : DataCreatorBase(name), baseline(pedestal), width(RMS * std::sqrt(12.))
  {}

  /// Creates and returns the data sample
  virtual InputData_t create(size_t size) override
  {
    std::uniform_real_distribution<float> noise(baseline - width, baseline + width);
...
public:
  ...
  GaussianNoiseCreator(std::string name, float sigma, float mu = 0.)
    : DataCreatorBase(name), mean(mu), stdev(sigma)
  {}
...
public:
  ...
  SineWaveCreator(std::string name, float new_period, float new_amplitude)
    : DataCreatorBase(name), period(new_period), amplitude(new_amplitude)
  {}
...
public:
  ...
  virtual InputData_t create(size_t size) override
  {
    std::uniform_int_distribution<short> uniform(std::numeric_limits<short>::min(),
...
void RunDataCompressionTest(std::string id, const std::vector<short>& data, raw::Compress_t mode)
{
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(data.begin(), data.end(), data_again.begin(), data_again.end());
