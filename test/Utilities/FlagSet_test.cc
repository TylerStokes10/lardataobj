...
void CheckFlags(FlagSet const& flags,
                ...
                std::set<typename FlagSet::Flag_t> const& set)
{
  ...
  for (FlagIndex_t i = 0; i <= flags.capacity(); ++i) {
    ...
    if (!isFlag) {
      ...
      BOOST_CHECK_THROW(flags.test(flag), typename FlagSet::FlagNotDefinedError);
    }
    else {
...
void FlagSetTest()
{
  ...
  std::for_each(indices.begin(), indices.end(), [&defined](auto flag) { defined.insert(flag); });
  std::for_each(indices.begin(), indices.end(), [&set](auto flag) { set.insert(flag); });
  std::cout << "  => " << flags << std::endl;
  CheckFlags(flags, defined, set);
  ...
  std::for_each(indices.begin(), indices.end(), [&defined](auto flag) { defined.insert(flag); });
  std::for_each(indices.begin(), indices.end(), [&set](auto flag) { set.erase(flag); });
  std::cout << "  => " << flags << std::endl;
  CheckFlags(flags, defined, set);
...
void BitMaskCombineTests()
{
  ...
  static_assert(maskA + maskB == maskMAB, "maskA + maskB failure");
  ...
  static_assert(Mask_t::mergeIntoMask(maskB, maskA) == maskMBA,
                "mergeIntoMask(Mask_t, Mask_t) failure");
  static_assert(maskB + maskA == maskMBA, "maskB + maskA failure");
  ...
  static_assert(maskA + bitsB == maskMAbB, "maskA + bitsB failure");
  ...
  static_assert((maskA | maskB) == maskCAB, "maskA | maskB failure");
  ...
  static_assert(Mask_t::combineWithMask(maskB, maskA) == maskCBA,
                "combineWithMask(Mask_t, Mask_t) failure");
  static_assert((maskB | maskA) == maskCBA, "maskB | maskA failure");
  ...
  static_assert((maskA | bitsB) == maskCAbB, "maskA | bitsB failure");

  constexpr Mask_t maskCbBA(Mask_t::fromValues, 0b011'011'111, 0b001'001'111);
  static_assert((bitsB | maskA) == maskCbBA, "bitsB | maskA failure");
  ...
  static_assert((maskA & bitsB) == maskIAbB, "maskA & bitsB failure");
  ...
  static_assert(Mask_t::unsetMask(maskA, maskB) == maskUAB, "unsetMask(Mask_t, Mask_t) failure");
  ...
  static_assert(Mask_t::unsetMask(maskB, maskA) == maskUBA, "unsetMask(Mask_t, Mask_t) failure");
  ...
  static_assert(Mask_t::unsetMask(maskA, bitsB) == maskUAbB, "unsetMask(Mask_t, Bits_t) failure");
  ...
  static_assert(Mask_t::negateMask(maskA) == maskNA, "negateMask(Mask_t) failure");
  ...
  static_assert(Mask_t::negateMask(bitsB) == maskNbB, "negateMask(Bits_t) failure");
