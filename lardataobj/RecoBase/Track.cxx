...
namespace recob {
  ...
  std::ostream& operator<<(std::ostream& stream, Track const& a)
  {
    ...
    stream << std::setiosflags(std::ios::fixed) << std::setprecision(3) << "\n Track ID "
           << std::setw(4) << std::right << a.ID() << " Theta = " << std::setw(6) << std::right
           << a.Theta() << " Phi = " << std::setw(6) << std::right << a.Phi()
           << "\n  StartCosines : ( " << start.X() << " ; " << start.Y() << " ; " << start.Z()
           << ")  EndCosines : ( " << end.X() << " ; " << end.Y() << " ; " << end.Z() << ")"
           << "\n  #Position and Direction = " << std::setw(5) << std::right
           << a.NumberTrajectoryPoints();
    stream << std::endl;
  ...
  {
    return tracking::Plane::Local5DToGlobal6DCovariance(
      fCovVertex, HasMomentum(), fTraj.VertexMomentumVector(), fTraj.VertexDirection());
  }

  Track::SMatrixSym66 Track::EndCovarianceGlobal6D() const
  {
    return tracking::Plane::Local5DToGlobal6DCovariance(
      fCovEnd, HasMomentum(), fTraj.EndMomentumVector(), fTraj.EndDirection());
  }
