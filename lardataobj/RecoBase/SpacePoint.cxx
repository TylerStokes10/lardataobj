...
namespace recob {
  ...
  //----------------------------------------------------------------------
  SpacePoint::SpacePoint() : fID(-1), fXYZ{0.0}, fErrXYZ{0.0}, fChisq(0.) {}

  //----------------------------------------------------------------------
  SpacePoint::SpacePoint(Double32_t const* xyz, Double32_t const* err, Double32_t chisq, int id)
    : fID(id), fChisq(chisq)
  {
    for (int i = 0; i < 3; ++i)
      fXYZ[i] = xyz[i];
  ...
  std::ostream& operator<<(std::ostream& o, const SpacePoint& a)
  {
    ...
    o << " SpacePoint ID " << std::setw(5) << std::right << a.ID() << " (X,Y,Z) = (" << std::setw(5)
      << std::right << a.XYZ()[0] << " , " << std::setw(5) << std::right << a.XYZ()[1] << " , "
