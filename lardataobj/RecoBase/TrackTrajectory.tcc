                                  ...
                                  std::string indentFirst) const
{
  ...
  if (NPoints() < 2) {
    out << indentFirst << "invalid track trajectory with " << NPoints() << " points";
    return;
  }

  //----------------------------------------------------------------------------
  out << indentFirst << "track trajectory with " << NPoints() << " points at "
  ...
  if (nInvalidPoints > 0) out << " (with " << nInvalidPoints << " invalid points)";
  ...
  switch (verbosity) {
  case 4: nPrintedPoints = 10; break;
  case 5: nPrintedPoints = 20; break;
  case 6:
  default: nPrintedPoints = nPoints; break;
  } // switch
  ...
  for (unsigned int step = 0; step < nPrintedPoints; ++step) {
    ...
    if (HasValidPoint(iPoint)) {
      out << " at " << util::manip::vector3D(LocationAtPoint(iPoint)) << " cm, "
          << util::manip::vector3D(MomentumVectorAtPoint(iPoint));
      if (HasMomentum()) out << " GeV/c";
    }
    out << " " << FlagsAtPoint(iPoint);
