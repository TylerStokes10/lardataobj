                             ...
                             std::string indentFirst) const
{
  ...
  if (NPoints() < 2) {
    out << indentFirst << "invalid trajectory with " << NPoints() << " points";
    return;
  }

  //----------------------------------------------------------------------------
  out << indentFirst << "trajectory with " << NPoints() << " points at "
      << util::manip::vector3D(Start()) << " cm toward " << util::manip::vector3D(StartDirection());
  ...
  //----------------------------------------------------------------------------
  out << indentFirst << "\n"
      << indent << "ends at " << util::manip::vector3D(End()) << " cm toward "
      << util::manip::vector3D(EndDirection());
  if (HasMomentum()) out << " with momentum " << EndMomentum() << " GeV/c";

  if (verbosity <= 1) return;
  //----------------------------------------------------------------------------
  out << " running " << Length() << " cm long";

  if (verbosity <= 2) return;
  //----------------------------------------------------------------------------

  out << "\n"
      << indent << "starting with theta " << Theta() << " rad, phi " << Phi()
      << " rad; zenith: " << ZenithAngle() << " rad, azimuth: " << AzimuthAngle() << " rad";
  ...
  switch (verbosity) {
  case 4: nPrintedPoints = 9; break;
  case 5: nPrintedPoints = 19; break;
  case 6:
  default: nPrintedPoints = nPoints - 2; break;
  } // switch
  ...
  for (unsigned int step = 1; step <= nPrintedPoints; ++step) {
    ...
    out << "  [#" << iPoint << "] at " << util::manip::vector3D(LocationAtPoint(iPoint)) << " cm, "
...
void recob::details::legacy::FillVector(Vect const& source, std::vector<double>& dest)
{
  dest.resize(3);
  dest[0] = source.X();
  dest[1] = source.Y();
...
template <typename Vect>
void recob::details::legacy::FillVector(Vect const& source, double* dest)
{
