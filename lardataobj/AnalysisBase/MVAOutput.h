...
namespace anab {
  ...
  public:
    ...
    float at(size_t index) const
    {
      if (index < N) { return fData[index]; }
        ...
        throw cet::exception("FeatureVector") << "Index out of range: " << index << std::endl;
      }
    }
  ...
  private:
    ...
    void set(std::vector<float> const& values)
    {
      if (values.size() == N) {
        ...
        throw cet::exception("FeatureVector")
          << "Expected length: " << N << ", provided: " << values.size() << std::endl;
      }
    }
  ...
  public:
    ...
    MVADescription(std::string const& outputInstance,
                   std::vector<std::string> const& outputNames = std::vector<std::string>(N, ""))
      : fDataTag("")
      , // initialize with empty data tag, so it should be assigned later
    ...
    {
      o << "MVADescription: prepared for " << a.fDataTag << ", instance name " << a.fOutputInstance
        << ", " << N << " outputs:" << std::endl;
      for (size_t i = 0; i < N; ++i) {
        o << " " << a.fOutputNames[i] << std::endl;
      }
      return o;
    }
    ...
    void setDataTag(const std::string& tag)
    {
      if (fDataTag.empty()) { fDataTag = tag; }
        ...
        throw cet::exception("MVADescription")
    ...
    const std::string& outputName(size_t index) const
    {
      if (index < N) { return fOutputNames[index]; }
        ...
        throw cet::exception("MVADescription") << "Index out of range: " << index << std::endl;
      }
    }
    void setOutputNames(std::vector<std::string> const& outputNames)
    {
      if (outputNames.size() <= N) {
        for (size_t i = 0; i < outputNames.size(); ++i) {
          fOutputNames[i] = outputNames[i];
        }
      }
      else {
        throw cet::exception("FeatureVector") << "Expected max length of outputNames: " << N
    ...
    {
      for (size_t i = 0; i < N; ++i) {
        if (fOutputNames[i] == name) { return i; }
      }
      return -1; // not found
    }
